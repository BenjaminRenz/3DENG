 ...
 * \param pack32toogle
 *        If pack32toggle is set, the returned ImageData.dataP must be read as uint32_t, where the lowest 8 bytes are the first color in the outputFormatString.
 *        If it is not set, the ImageData.dataP is written as uint8_t and the first byte in ImageData.dataP corresponds with the first color in the outputFormatString,
 *        independent of the system endianness.
 * \return ImageData.dataP
 *        The function will heap allocate the necessary memory and will write this memory address into ImageData.dataP.
