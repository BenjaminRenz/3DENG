...
void eng_load_static_models(struct VulkanRuntimeInfo* vkRuntimeInfoP,struct DynamicList* DaePaths){
    //struct eng3dObject ObjectToBeLoaded;    TODO remove this global hack
    for(uint32_t DaePathIdx=0;DaePathIdx<DaePaths->itemcnt,DaePathIdx++){
        struct DynamicList* FilePathUtf32DlP=((struct DynamicList**)(DaePaths->items))[DaePathIdx];
        char* FilePathString=Dl_utf32_toString(FilePathUtf32DlP);
        daeLoader_load(FilePathString,
    }
...
int main(int argc, char** argv){
    ...
    struct DynamicList* ModelPathsNestedDlP=DlAlloc(sizeof(struct DynamicList*),dlType_NestedDl,0,NULL);
    DlAppend(&ModelPathsNestedDlP,1,Dl_utf32_fromString("./res/cube.dae"));
    DlAppend(&ModelPathsNestedDlP,1,Dl_utf32_fromString("./res/cylinder.dae"));
    eng_load_static_models(&engVkRuntimeInfo,ModelPathsNestedDlP);          //depends on eng_createCommandPool and creates vertex buffer
